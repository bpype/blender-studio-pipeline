...
RELEASE_TITLE = "Project Tools"
RELEASE_VERSION = "project-tools"
RELEASE_DESCRIPTION = "Latest Release of Project Tools Script"

ZIP_NAME = "project-tools"
...
def create_latest_addons_zip(name: str, temp_dir: Path):
    """Generate a release for the project-tools directory."""
    output_dir = Path(temp_dir).joinpath(name)
    output_dir.mkdir()
    # Path to the actual project-tools directory
    project_tools_dir = Path(__file__).parents[1].joinpath("project-tools")
    if not project_tools_dir.exists():
        print(f"Directory not found: {project_tools_dir}")
        sys.exit(1)
