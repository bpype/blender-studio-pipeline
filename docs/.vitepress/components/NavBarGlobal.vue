...
export default {
  ...
  methods: {
    dropdownToggle(event) {
      let dropdownToggleBtn = event.currentTarget;
      ...
      dropdownMenuItem.classList.toggle(this.isVisibleClass);
    }
  },
  mounted() {
    ...
    for (let i = 0; i < dropdownToggles.length; i++) {
      dropdownToggles[i].addEventListener('click', this.dropdownToggle, false);
    }
    ...
    document.onclick = (e) => {
      ...
      if (!e.target.classList.contains('js-dropdown-toggle') || (targetMenuElement && targetMenuElement.classList.contains(this.isVisibleClass))) {
        ...
        for (let i = 0; i < dropdownMenus.length; i++) {
          if (dropdownMenus[i].id && dropdownMenus[i].id !== targetMenuId) {
            dropdownMenus[i].classList.remove(this.isVisibleClass, 'active');
          }
        }
