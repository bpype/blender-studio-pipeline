...
- If you want to relink an Armature constraint, a helper bone will normally be created to hold the constraint. If you want to avoid this, put `NOHLP` after the constraint name, eg. `IK-Armature_NOHLP`. The underscore is not required, but note that you must not use a dash (-).
