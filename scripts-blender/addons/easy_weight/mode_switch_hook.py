...
def on_weight_paint_enter():
    ...
    if not armature:
        return
    if not prefs.always_reveal_armature:
        return
    ...
    if (
        not armature.visible_get()
        and armature.name not in context.scene.collection.objects
    ):
...
def on_weight_paint_leave():
    ...
    if "arm_disabled" not in wp_toggle_as_dict:
        # Nothing to restore: on enter, revealing the armature was skipped, either
        # because there was no deforming armature, or "Always Reveal Armature" was disabled.
        return
...
def set_brush_prefs():
    ...
    prefs.global_unified_strength = prefs.global_unified_strength
...
def detect_mode_switch(scene=None, depsgraph=None):
    ...
    # Only the last two entries are ever needed; keep the list from growing forever.
    del mode_history[:-2]
    if mode_history[-1] == "PAINT_WEIGHT" and (
        len(mode_history) == 1 or mode_history[-2] != "PAINT_WEIGHT"
    ):
        suspend_hook = True
        try:
        ...
        len(mode_history) > 1
        and mode_history[-2] == "PAINT_WEIGHT"
        and mode_history[-1] != "PAINT_WEIGHT"
    ):
        suspend_hook = True
        try:
