...
class EASYWEIGHT_MT_PIE_easy_weight(Menu):
    ...
    def draw(self, context: Context):
        ...
        # 8) <v Unified Strength
        pie.prop(prefs, "global_unified_strength", icon="BRUSHES_ALL")
...
class EASYWEIGHT_MT_PIE_easy_weight_operators(Menu):
    ...
    def draw(self, context: Context):
        ...
        # 4) ^ Smooth Weights.
        op = pie.operator("object.vertex_group_smooth", icon="MOD_SMOOTH")
...
class EASYWEIGHT_MT_PIE_easy_weight_overlays(Menu):
    ...
    def draw(self, context: Context):
        ...
        # 1) < X-Ray
        if (
            context.pose_object and
            hasattr(context.pose_object, 'show_in_front') and
            context.pose_object.show_in_front
        ):
            pie.prop(
                context.pose_object,
                "show_in_front",
        ...
        if context.pose_object:
            ...
            box.row(align=True).prop(
                context.pose_object.data, "display_type", expand=True
            )
        else:
            pie.separator()
