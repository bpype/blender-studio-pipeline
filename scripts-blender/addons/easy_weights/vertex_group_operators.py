...
class DeleteUnselectedDeformGroups(WeightPaintOperator):
    ...
    def execute(self, context):
        ...
        self.report(
            {'INFO'}, f"Deleted {len(deleted_names)} unselected deform groups.")
        return {'FINISHED'}
...
class DeleteUnusedVertexGroups(Operator):
    ...
    def execute(self, context):
        ...
        self.report(
            {'INFO'}, f"Deleted {len(deleted_names)} unused non-deform groups.")
        return {'FINISHED'}
...
def get_symmetry_mapping(*, obj: Object, axis='X'
    ...
    def zero_or_more(x): return x >= 0
    def zero_or_less(x): return x <= 0
...
def symmetrize_vertex_group(*, obj: Object, vg_name: str, symmetry_mapping: Dict[int, int], right_to_left=False
                            ):
    """
...
class SymmetrizeVertexGroups(Operator):
    ...
    groups: EnumProperty(
        name="Subset", description="Subset of vertex groups that should be symmetrized", items=[
            ('ACTIVE', 'Active', 'Active'), ('BONES',
                                             'Selected Bones', 'Selected Bones'), ('ALL', 'All', 'All')
        ]
    )
        ...
        if self.groups == 'SELECTED':
            ...
            for vg_name in context.context.selected_pose_bones:
                ...
                if flipped_vg in vgs:
                    self.report(
                        {'ERROR'}, f'Both sides selected: "{vg.name}" & "{flipped_vg.name}". Only one side should be selected.')
                    return {'CANCELLED'}
                vgs.append(vg)

            vgs = [obj.vertex_groups.get(pb.name)
                   for pb in context.selected_pose_bones]
