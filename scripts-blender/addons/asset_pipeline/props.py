...
from bpy.types import ID, Context, Object, PropertyGroup, Scene
...
class AssetTransferData(PropertyGroup):
...
class AssetTransferDataTemp(PropertyGroup):
...
class AssetPipeline(PropertyGroup):
    ...
    ui_ownership_mode: EnumProperty(
        name="Ownership Display Mode",
        items=[
            ('OBJECT', 'Object', "Show ownership data of the active object (in the 3D View)", 'OBJECT_DATAMODE', 0),
            ('COLLECTION', 'Collection', "Show ownership data of the active collection (in the Outliner)", 'OUTLINER_COLLECTION', 1),
        ]
    )
...
def register():
    Object.transfer_data_ownership = CollectionProperty(type=AssetTransferData)
    Scene.asset_pipeline = PointerProperty(type=AssetPipeline)
...
def unregister():
    del Object.transfer_data_ownership
    del Scene.asset_pipeline
    del ID.asset_id_owner
    del ID.asset_id_surrender
